...
AClientMonster::AClientMonster()
{
	HpBar = CreateDefaultSubobject<UWidgetComponent>(TEXT("HPBAR"));
	////메쉬에 붙여준다..?
	////COmponents/WidgetComponent추가
	HpBar->SetupAttachment(GetMesh());
	HpBar->SetRelativeLocation(FVector(0.0f, 0.0f, 200.0f));
	HpBar->SetWidgetSpace(EWidgetSpace::Screen);

	////특정 경로에 있는 위젯 컴포넌트를 만들 것이다.
	////블루 프린트로 만들 애를 클래스로 쓸 때는 _C를 붙여줘야 한다.
	static ConstructorHelpers::FClassFinder<UUserWidget> UW(TEXT("WidgetBlueprint'/Game/ServerClient/UI/WBP_Hpbar.WBP_HpBar_C'"));
	if (UW.Succeeded())
	{
...
void AClientMonster::PreInitializeComponents()
{
	HpBar->InitWidget();
	auto HpWidget = Cast<UMyChracterWidget>(HpBar->GetUserWidgetObject());
	if (HpWidget)
	{
...
void AClientMonster::Tick(float DeltaTime)
{
	...
	while (false == GetMessage()->IsEmpty())
	{
		...
		if (MessageId::MonsterUpdate == Message->GetId<MessageId>())
		{
			...
			//HpWidget->UpdateHp(static_cast<float>(UpdateMessage->Data.Hp) / 200.0);
			UE_LOG(LogTemp, Log, TEXT("HP: %d"), UpdateMessage->Data.Hp);

			Hp = UpdateMessage->Data.Hp;
		...
		else if (MessageId::ObjectDestroy == Message->GetId<MessageId>())
		{
			...
			//진짜로 삭제를 한다면.....AllObject_, TypeOfAllObject_, CirCleCollisionGroup에서 삭제되야 하는데...
			//
...
void AClientMonster::Destroyed()
{
	//이러면 AClientCharacter의 Destory()냐, AActor의 Destory()냐.....
	Super::Destroyed();
...
int AClientMonster::GetHp()
{
	return Hp;
}

int AClientMonster::GetMaxHp()
{
	return MaxHp;
}
