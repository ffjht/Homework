...
void AClientPlayCharacter::SendPlayerUpdatePacket()
{
	...
	if (UpdateMsg.Data.State == static_cast<int>(ClientAnimationType::Attack))
	{
		UE_LOG(LogTemp, Log, TEXT("PLAYER Attack"));
		FDateTime CurTime = UKismetMathLibrary::UtcNow();
		UpdateMsg.Time.Year = CurTime.GetYear();
		UpdateMsg.Time.Month = CurTime.GetMonth();
		UpdateMsg.Time.Date = CurTime.GetDay();
		UpdateMsg.Time.Hour = CurTime.GetHour();
		UpdateMsg.Time.Minute = CurTime.GetMinute();
		UpdateMsg.Time.Second = CurTime.GetSecond();
		UpdateMsg.Time.Millisecond = CurTime.GetMillisecond();
	}
...
void AClientPlayCharacter::Tick(float DeltaTime)
{
	...
	while (false == GetMessage()->IsEmpty())
	{
		...
		else if (MessageId::PlayerUpdate == Message->GetId<MessageId>())
		{
			...
			//이러면 형 변환을 해주는 것이 맞나?
			//UE_LOG(LogTemp, Log, TEXT("PLAYER HP: %d"), UpdateMessage->Data.Hp);
			//UE_LOG(LogTemp, Log, TEXT("Level: %d"), UpdateMessage->Data.Level);
		}
		else 
		{
			int a = 0;
